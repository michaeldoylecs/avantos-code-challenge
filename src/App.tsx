...
function App() {
  ...
  const setFromActionBlueprintGraph = useActionBlueprintGraphStore(
    (state) => state.setFromActionBlueprintGraph,
  );
  ...
  const formPropertySelectCallback = (name: FieldSchemaPropertyName) => () => {
    ...
    while (visit_stack.length > 0) {
      ...
      if (curr_node != null) {
        const form_properties = getForm(curr_node.data.component_id)
            ...
            <div className="h-screen">
              <main className="flex flex-col p-1">
                <div className="flex flex-col p-1">
                  ...
                  <NodeDisplay
                    nodes={getNodes()}
                    onClickCallback={nodeOnClickCallback}
                  />
                </div>
                <Separator />
                {!!selectedNode && (
                  <FormTable
                    key={selectedNode.id}
                    node={selectedNode}
                    onClickCallback={formPropertySelectCallback}
                  />
                )}
              </main>
            </div>
