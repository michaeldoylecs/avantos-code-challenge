...
export default function FormTable({node, onClickCallback} : FormTableProps) {
  ...
  const schema_properties = form && Object.keys(form.field_schema.properties) || []

  const getPrefillStringIfExists = (field_property: FieldSchemaPropertyName) => {
    if (prefillMap == null || prefillMap[node.id] == null || prefillMap[node.id][field_property] == null) {
      return ''
    }
    ...
    <Table>
      ...
      <TableBody>
        {schema_properties.map((property, _) => (
          <TableRow key={property} className="hover:cursor-pointer" onClick={onClickCallback(property)}>
            <TableCell className="text-left">{property}</TableCell>
            <TableCell className="text-left">{getPrefillStringIfExists(property)}</TableCell>
            <TableCell className="text-left">{ prefillMap[node.id] && prefillMap[node.id][property] && <Button className="p-2" onClick={() => removePrefill(node.id, property)}>Remove</Button>}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
