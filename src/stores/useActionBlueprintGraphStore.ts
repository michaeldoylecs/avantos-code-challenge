...
export const useActionBlueprintGraphStore = create<ActionBlueprintGraphStore>(
  (set, get) => ({
    actionBlueprintGraph: null,
    setFromActionBlueprintGraph: (graph: ActionBlueprintGraph) =>
      set({ actionBlueprintGraph: graph }),
    getNode: (node_id: NodeID) =>
      get().actionBlueprintGraph?.nodes.find((node) => node.id == node_id) ||
      null,
    getNodes: () => get().actionBlueprintGraph?.nodes || [],
    getForm: (id: string) =>
      get().actionBlueprintGraph?.forms.find((form: Form) => form.id === id) ||
      null,
  }),
);
